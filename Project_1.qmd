...
We created a few helper functions to ensure separation of concerns and modularity in our design. The implementation of these functions is detailed below
                             ...
                             survey = "acs1", show_call = FALSE)
{
  ...
  # Extract times into a list of parsed times and chain together
  # to return 24hr formatted midpoint times
  for (varname in datetime_var_chk){
    if (varname == "JWAP"){
      parsed_data <- parsed_data |>
        mutate(
          JWAP = as.character(JWAP),
          time_parsed = map(JWAP, extract_times),
          first_24 = map_chr(time_parsed, "first_24"), 
          second_24 = map_chr(time_parsed, "second_24"),  
      ...
      parsed_data <- parsed_data |>
        mutate(
          ...
          time_parsed = map(JWDP, extract_times),  
          first_24 = map_chr(time_parsed, "first_24"),  
          second_24 = map_chr(time_parsed, "second_24"),  
          midpoint = map_chr(time_parsed, calculate_midpoint)) |>
      ...
      parsed_data <- parsed_data |>
        mutate(
          ...
          jwmnp_n = suppressWarnings(as.numeric(jwmnp_c)),  
...
- tests the function for all values and default values (`NULL` case for `num_vars` and `chr_vars`)
